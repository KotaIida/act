...
class PickAndPutInCardboardVPolicyFranka(BasePolicy):
    def __init__(self, inject_noise=False, careful=False, quick=False):
        self.inject_noise = inject_noise
        self.step_count = 0
        self.left_trajectory = None
    ...
    def generate_trajectory(self, ts_first):
        ...
        gripper_pick_quat = Quaternion(init_mocap_pose_right[3:])
        gripper_pick_quat_obj = Quaternion([1, 0, 0, 0]) * Quaternion(axis=[0, 1, 0], degrees=360)
        ...
        self.right_trajectory = [
            ...
            {"t": 180, "xyz": obj_xyz + np.array([0, 0.015, -0.049]), "quat": gripper_pick_quat_obj.elements, "gripper": 1}, # approach the cushion
            {"t": 220, "xyz": obj_xyz + np.array([0, 0.015, -0.049]), "quat": gripper_pick_quat_obj.elements, "gripper": 0}, # close gripper
            {"t": 280, "xyz": obj_xyz + np.array([0, -0.1, -0.049]), "quat": gripper_pick_quat_obj.elements, "gripper": 0}, # go back
...
class PickAndPutInCardboardHPolicyFranka(BasePolicy):
    ...
    def generate_trajectory(self, ts_first):
        ...
        if -135 <= np.rad2deg(dst_angle) < 45:
            dst_xyz[:2] += np.array([[np.cos(dst_angle), -np.sin(dst_angle)], [np.sin(dst_angle), np.cos(dst_angle)]]) @ np.array([self._x_offset, -self._y_offset])
        else:
            dst_xyz[:2] += np.array([[np.cos(dst_angle), -np.sin(dst_angle)], [np.sin(dst_angle), np.cos(dst_angle)]]) @ np.array([-self._x_offset, -self._y_offset])

        if -135 <= np.rad2deg(dst_angle) < 45:
            dst_quat = Quaternion(axis=[0, 0, 1], degrees=np.rad2deg(dst_angle) + 90)
        elif 45 <= np.rad2deg(dst_angle) < 180:
            dst_quat = Quaternion(axis=[0, 0, 1], degrees=np.rad2deg(dst_angle) - 90)
        else:
            dst_quat = Quaternion(axis=[0, 0, 1], degrees=np.rad2deg(dst_angle) + 90 + 180)            
